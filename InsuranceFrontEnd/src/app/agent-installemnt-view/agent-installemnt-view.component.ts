...
@Component({
  selector: 'app-agent-installemnt-view',
  templateUrl: './agent-installemnt-view.component.html',
  styleUrls: ['./agent-installemnt-view.component.css']
})
export class AgentInstallemntViewComponent {
    ...
    this.customer.makePayment(payment).subscribe({
      next: (res) => {
        ...
        this.dialog.open(PaymentReciptComponent, {
          data: payment// Pass payment data to the dialog
        });
      },
      error: (err: HttpErrorResponse) => {
