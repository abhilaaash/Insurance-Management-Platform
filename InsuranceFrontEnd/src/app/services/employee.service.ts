...
export class EmployeeService {
  ...
  getFilterEmployees(pgNo?: number, pgSize?: number, searchQuery?: any) {
    var searchUrl = "https://localhost:7124/api/Employee/get?PageNumber=" + pgNo + "&PageSize=" + pgSize;
    if (searchQuery !== undefined) {
      ...
      if (typeof searchQuery === 'number') {
        ...
        if (isEmail) {
          searchUrl += `&Email=${searchQuery}`;
        } else if (isPhoneNumber) {
          searchUrl += `&PhoneNumber=${searchQuery}`;
        } else {
          searchUrl += `&FirstName=${searchQuery}`;
        }
      }
